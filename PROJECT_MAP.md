...
- Footer "Blog" link hidden (`Footer.tsx`, commented placeholder left) until real posts exist — `/blog` routes themselves remain live and in the sitemap.
- Everything committed to `main` and pushed to github.com/yelramy/eweeha (commit `e744e9a`). Push note: Windows credential cache defaults to the `yelramy-titan` account which lacks repo access — push with `git -c credential.helper= -c credential.helper='!gh auth git-credential' push` after `gh auth switch --user yelramy`.

**Jul 4 2026 (later) — mobile hero visibility fix, sticky bottom bar removed:**
- Mobile hero photo was invisible under uniform washes (`cream-50/80` light / `gray-900/85` dark + image `dark:opacity-45`). Now: top-heavy vertical gradient on mobile (`from-cream-50/90 via-cream-50/55 to-cream-50/30`, dark `from-gray-900/85 via-.../50 to-.../25`) — strong wash behind title, car shows through below; image `dark:opacity-70` on mobile only (`sm:dark:opacity-45` keeps desktop dark unchanged). Desktop gradients untouched.
- `MobileContactBar` (sticky bottom Call Now / WhatsApp bar) DELETED (user: page too busy; green FAB is enough). `ContactButtons.tsx` now renders only `WhatsAppButton`; FAB moved to `bottom-6` on all screens (was `bottom-20` above the bar); `BackToTop` bumped to `bottom-24 md:bottom-28` so it stacks above the FAB instead of overlapping it.
- Verified in emulated 390px mobile, light + dark. `tsc --noEmit` clean.

**Jul 4 2026 (night 2) — hero made rich in all modes + state-of-the-art SEO / AI visibility:**
- **Hero photo now rich, not washed** (user request). `HomeClient.tsx`: image `dark:opacity-85` mobile / `sm:dark:opacity-60` desktop (was 45); mobile wash gradient `from-cream-50/85 via-45 to-15` (dark `gray-950/85→45→15`); desktop gradients lightened (`via-50`, right side `to-gray-950/10` in dark). Text contrast compensated: hero paragraph `text-charcoal-600 dark:text-gray-200`, trust chips `font-medium text-charcoal-500 dark:text-gray-200`, WhatsApp outline button bg `cream-50/85 dark:gray-950/60`. Verified via screenshots in ALL FOUR states (mobile/desktop × light/dark) — car visible and text crisp everywhere.
- **Root JSON-LD consolidated to ONE `@graph`** (`layout.tsx` + `seoManager.generateStructuredData`): previously TWO competing business entities (`#organization` AutoRental + `#business` LocalBusiness) — now a single `#organization` node (`@type: [AutoRental, LocalBusiness]`) with geo, slogan, openingHoursSpecification, full areaServed city list, makesOffer with service URLs, priceRange $250-$1500, aggregateRating (only when real reviews exist) + a `#website` WebSite node referencing it. `generateWebSiteStructuredData()` added to seoManager. `metadataBase` set in layout.
- **FAQPage JSON-LD added to all 4 service pages** (via existing `FAQSchema` component — was defined but never used); duplicate FAQPage removed from `FAQClient.tsx` (page.tsx already emits it — was shipping twice). `ServiceSchema`/`LocalBusinessSchema` bogus `$50-$120` defaults → `$250-$1500`; `LocalBusinessSchema` now carries `@id: #organization` so about/reviews pages merge into the root entity instead of duplicating it.
- **robots.ts**: AI crawler allowlist extended (added Applebot, meta-externalfetcher, CCBot, Amazonbot, DuckAssistBot, MistralAI-User, Bytespider); tokenized private pages `/quote/*` + `/review/*` disallowed; llms.txt removed from the Sitemap list (not a sitemap).
...
**Jul 5 2026 — SEO + social share cards upgraded (user request):**
- **Google search:** Root layout uses `title.template` (`%s | Eweeha`) so titles no longer double-append; homepage `page.tsx` now uses `generateMetadata()` with full Open Graph + Twitter large-image tags (was missing `og:image`). Organization JSON-LD logo enriched (`@id`, `contentUrl`, caption); `WebSite` node now includes `og-image.jpg`. Manifest icons/theme aligned to wine `#742F38` + `logo.png`.
- **Social sharing:** New shared template `src/lib/og-image-template.tsx` + `src/lib/ogImageAssets.ts` — premium 1200×630 card (hero photo, logo badge, wine/gold overlays, eweeha.com footer). Both `/og-image.jpg` (default for all pages) and `/og-image?title=…` (dynamic) use it. Admin `SocialPreview` default image → `/og-image.jpg`.
- **Files:** `seoManager.ts` (`formatPageTitle`, `pageTitleSegment`, stronger default description/keywords), `layout.tsx`, `page.tsx`, `manifest.ts`, both OG routes.
