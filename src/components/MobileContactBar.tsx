...
export default function MobileContactBar({ quoteData }: MobileContactBarProps) {
  ...
  const phoneNumber = appConfig?.contact?.phone || '+961-70-971-841'
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-[#742F38] border-t border-gray-200 shadow-[0_-4px_12px_rgba(0,0,0,0.15)]" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="flex">
        {/* Call Button */}
        <a
          href={`tel:${telLink}`}
          onClick={() => events.phoneClicked('mobile_bar')}
          className="flex-1 flex items-center justify-center gap-2 py-4 px-4 bg-[#742F38] hover:bg-[#5C262D] active:bg-[#4A1F25] text-white font-semibold transition-colors duration-150 active:scale-[0.98]"
          aria-label="Call us"
        >
