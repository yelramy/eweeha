...
const MAX_PER_CAR = 20

/**
 * Tap-to-pick convoy builder: one scrollable list (no pages), big swipeable
 * photos per car, quantity per car (the same model can be booked several
 * times), then send the selection to WhatsApp.
 */
export default function ConvoyPicker({ isOpen, onClose, vehicles }: ConvoyPickerProps) {
  ...
  useEffect(() => {
    ...
    // Hides the floating WhatsApp/back-to-top buttons so they don't sit on top
    // of the picker's own WhatsApp CTA (see globals.css).
  ...
  const setQuantity = (id: string, value: number) => {
    setQuantities((prev) => {
      const next = { ...prev }
      const clamped = Math.min(MAX_PER_CAR, Math.max(0, value))
  ...
  const add = (id: string) => setQuantity(id, (quantities[id] ?? 0) + 1)
  const remove = (id: string) => setQuantity(id, (quantities[id] ?? 0) - 1)

  const selectedVehicles = vehicles
    .map((v) => ({ vehicle: v, count: quantities[String(v.id)] ?? 0 }))
    .filter((entry) => entry.count > 0)
  const totalCars = selectedVehicles.reduce((sum, e) => sum + e.count, 0)
  const selectedFromTotal = selectedVehicles.reduce(
  ...
  const handleSend = () => {
    ...
    const names = selectedVehicles
      .map((e) => (e.count > 1 ? `* ${e.count} x ${e.vehicle.name}` : `* ${e.vehicle.name}`))
      .join('\n')
    const when = date
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full sm:max-w-3xl sm:mx-4 max-h-[92dvh] sm:max-h-[85vh] bg-cream-50 dark:bg-gray-900 sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-3 border-b border-warm-200 dark:border-gray-700">
          <div>
            ...
            <p className="text-xs md:text-sm text-warm-600 dark:text-gray-400 mt-1">
              Tap a car to add it — tap again or use + to take more than one. {vehicles.length} cars in the fleet.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {filtered.map((vehicle) => {
                ...
                return (
                  <div
                    key={id}
                    className={`relative select-none rounded-xl overflow-hidden border-2 transition-all ${
                      selected
                  ...
                  >
                    ...
                    {selected && (
                      <span className="absolute top-2 right-2 min-w-7 h-7 px-2 rounded-full bg-primary-600 text-white text-sm font-semibold flex items-center justify-center shadow z-10 tabular-nums">
                        {count}
                      </span>
                    )}
                    <div
                      ...
                      {selected ? (
                        <div className="mt-2 flex items-center justify-between gap-2">
                          <button
                            type="button"
                            onClick={() => remove(id)}
                            className="w-9 h-9 p-0 shrink-0 rounded-full border border-warm-300 dark:border-gray-600 text-charcoal-600 dark:text-gray-200 flex items-center justify-center hover:bg-warm-100 dark:hover:bg-gray-700 transition-colors"
                            aria-label={`Remove one ${vehicle.name}`}
                          >
                            <MinusIcon className="w-4 h-4" />
                          </button>
                          <span className="text-sm font-semibold text-charcoal-600 dark:text-white tabular-nums" aria-live="polite">
                            {count}
                          </span>
                          <button
                            type="button"
                            onClick={() => add(id)}
                            disabled={count >= MAX_PER_CAR}
                            className="w-9 h-9 p-0 shrink-0 rounded-full bg-primary-600 text-white flex items-center justify-center hover:bg-primary-700 transition-colors disabled:opacity-40"
                            aria-label={`Add another ${vehicle.name}`}
                          >
                        ...
                        <button
                          type="button"
                          onClick={() => add(id)}
                          className="mt-2 w-full h-9 px-3 py-0 rounded-full border border-primary-600 text-primary-700 dark:text-primary-300 text-sm font-medium hover:bg-primary-50 dark:hover:bg-primary-900/30 transition-colors"
                        >
                          Add
                        </button>
                      )}
        ...
        <div
          className="px-5 py-4 border-t border-warm-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
        ...
        >
          <div className="text-sm text-warm-700 dark:text-gray-300">
            {totalCars === 0 ? (
              ...
              <>
                <span className="font-medium">{totalCars} car{totalCars > 1 ? 's' : ''}</span>
                {selectedFromTotal > 0 && (
          ...
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-full bg-[#25D366] hover:bg-[#1DA851] text-white text-sm font-semibold shadow-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
